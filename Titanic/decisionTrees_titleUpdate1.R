...
data = subset(data, select = c(2,3,5,6,7,8,10,12,13))					# only select the cols required 				# only select the cols required 
...
model = rpart(Survived ~ Pclass + Sex + Age + Fare + Embarked + Title, data=train, method='class')	# train the logistic regression model
plot(model)
...
actualTest$Fare[is.na(actualTest$Fare)] = predict(FareModel, actualTest[is.na(actualTest$Fare), ]) 
