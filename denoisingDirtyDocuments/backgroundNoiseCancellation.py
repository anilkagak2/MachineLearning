...
# Extract all reference patches from the clean image
print('Extracting reference patches...')
t0 = time()
patch_size = (7, 7)
data = extract_patches_2d(output, patch_size)
...
t0 = time()
data = extract_patches_2d(input, patch_size)
data = data.reshape(data.shape[0], -1)
...
for title, transform_algorithm, kwargs in transform_algorithms:
    print(title + '...')
    reconstructions[title] = input.copy()
    t0 = time()
    ...
    dt = time() - t0
    print('done in %.2fs.' % dt)
    show_with_diff(reconstructions[title], output, title + ' (time: %.1fs)' % dt)
